...
// Agenda Content Component
function AgendaContent({ theme }) {
  ...
  const getDayNumber = (date) => {
    const dayMap = { '12 JUL': '1ST', '13 JUL': '2ND', '14 JUL': '3RD' }
  ...
  const generateTimelinePoints = (events) => {
    const uniqueTimes = [...new Set(events.map(event => event.startTime))].sort()
  ...
  const scrollToEvent = (targetTime) => {
      ...
      scrollTimeout = setTimeout(() => {
        ...
        if (isScrollingDown && scrollPercentage >= 0.95) {
          ...
          if (nextDate) {
            ...
            // Reset scroll position and auto-scroll flag after transition
            setTimeout(() => {
              if (scrollContainer) {
